clc
clear
cd ('F:\MATLAB\work\generation_op\resimulation');
...
clear a b c astd auo ans i j k minus oricn orimn
 ...
 %假设检验3 h u1 u2 v1 v2为0，同时也为传统模型%
 ...
 LR(t,3)=(-2)*(para3logL0(t,1)-paralogL1(t,1));
 ...
 %假设检验4 u1 u2 v1 v2为0，同时也为imprint模型%
...
LR(t,4)=(-2)*(para4logL0(t,1)-paralogL1(t,1));
...
for i=1:3
 ...
 LR(t,5)=(-2)*(para5logL0(t,1)-paralogL1(t,1));
...
power(3)=numel(find(LR(:,3)>=11.070))/100;
power(4)=numel(find(LR(:,4)>=9.488))/100;
power(5)=numel(find(LR(:,5)>=3.841))/100;
