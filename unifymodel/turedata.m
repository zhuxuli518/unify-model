    clc
    clear
    cd ('F:\MATLAB\work\generation_op\re_turedata');
    motherdata=xlsread('董雷鸣榧树srap及表型数据汇总20130222.xls','母本标记');
    childdata=xlsread('董雷鸣榧树srap及表型数据汇总20130222.xls','子代标记');
    phenotypedata=xlsread('董雷鸣榧树srap及表型数据汇总20130222.xls','表形');
...
L1(jj,ii)=sum(log(sum(fpheno.*phichild,2)));
...
clear a b c astd auo 
 ...
 ii
  ...
  clear iastd iauo iestd ieuo chidata apm apo epm epo istd iuo 
    end
